...
%Void fraction, particle count rate and chord times-----------------------%
    [mode1air(j),mode1water(j),threshold1(j),C1(j),S1f]=thres(S1r,100);
    [mode2air(j),mode2water(j),threshold2(j),C2(j),S2f]=thres(S2r,100);  
...
%ROC filtering until no more data are rejected
